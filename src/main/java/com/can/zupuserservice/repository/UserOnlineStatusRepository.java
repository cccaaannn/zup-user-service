...
//    @Query("UPDATE UserOnlineStatus SET onlineStatus = :newStatus WHERE id in (SELECT User.userOnlineStatus FROM User WHERE id = :userId)")
    @Query(value = "UPDATE user_online_status SET status = :newStatus WHERE id in (SELECT user.user_online_status FROM user WHERE id = :userId)", nativeQuery = true)
    void updateUserOnlineStatus(@Param("userId") Long userId, @Param("newStatus") Integer newStatus);
